...
/** 8kHz → 16kHz.
 * NOTE: no longer used on the uplink — voice-server now sends native 8 kHz to
 * Pipecat and the backend adapts per engine (pipeline: 8 kHz direct to
 * Transcribe; nova-sonic: upsamples server-side). Kept for potential reuse /
 * tests; safe to remove if it stays unreferenced. */

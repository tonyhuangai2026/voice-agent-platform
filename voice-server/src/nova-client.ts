import {
  ...
  import  { availableTools, toolProcessor } from "./tools"
  ...
  export class S2SBidirectionalStreamClient {
    ...
    constructor(config: S2SBidirectionalStreamClientConfig) {
      ...
      this.bedrockRuntimeClient = new BedrockRuntimeClient({
        ...
        region: config.clientConfig.region || "us-west-2",
    ...
    private setupSessionStartEvent(sessionId: string): void {
      ...
      // Session start event - MUST be the first event in the queue
      // Add to the beginning of the queue (unshift) instead of the end (push)
      this.updateSessionActivity(sessionId);
    ...
    public setupPromptStartEvent(sessionId: string, voiceId?: string, customTools?: any[]): void {
      ...
      // Use custom voiceId if provided, otherwise use default
      const audioOutputConfig = voiceId
